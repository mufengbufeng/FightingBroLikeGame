...
namespace EF.Editor.SerializeComponentTool {

	public class SerializedComponentWindow : EditorWindow, IHasCustomMenu {

		#region MenuItem

		[MenuItem("EF/Serialize Component Tool/Open Binding Tool")]
		[MenuItem("Assets/EF/Serialize Component Tool/Open Binding Tool", false)]
		...
		[MenuItem("Assets/EF/Serialize Component Tool/Open Binding Tool", true)]
		...
		private bool SerializeObject(string ns, FieldObject fieldobj) {
			...
			while (sortedComponents.Count > 0) {
				...
				foreach (Assembly assembly in AppDomain.CurrentDomain.GetAssemblies()) {
					Type candidate = assembly.GetType(typeFullName, false)
						?? Type.GetType(typeFullName + "," + assembly.GetName().Name);
					if (candidate == null) { continue; }
					if (candidate.IsSubclassOf(tm)) {
						type = candidate;
						break;
					}
					ft = candidate;
