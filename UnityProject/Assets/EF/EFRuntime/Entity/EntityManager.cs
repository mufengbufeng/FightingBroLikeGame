...
namespace EF.Entity
{
    ...
    public sealed class EntityManager : AEFManager, IEntityManager
    {
        ...
        public override void Shutdown()
        {
            ...
            {
                if (asset != null && _resourceManager != null)
                {
                    _resourceManager.Release(asset);
                }
            }
            _assetCache.Clear();
        }
        ...
        /// </summary>
        /// <param name="entityAssetName">实体资源名称。</param>
        /// <returns>加载完成的实体 Prefab。</returns>
        private async UniTask<GameObject> LoadEntityAssetAsync(string entityAssetName)
        {
            // 优先从缓存中获取
            if (_assetCache.TryGetValue(entityAssetName, out GameObject cachedAsset))
            {
                return cachedAsset;
            }

            GameObject asset = await _resourceManager.Load<GameObject>(entityAssetName);
            ...
            // 缓存资源引用
            _assetCache[entityAssetName] = asset;
            return asset;
        }
